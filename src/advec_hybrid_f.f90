     ...
     do j=2,j1+1
        do i=2,i1+1
           ...
           if (lsmx) then ! field around this location is smooth -> use regular 5th order (upwind)
              sgn = sign(real(1.0,kind(u0)),u0(i,j,k))  ! set sgn, to account for different wind directions
           ...
           if (lsmy) then ! field around this location is smooth -> use regular 5th order (upwind)
              sgn = sign(real(1.0,(kind(v0))),v0(i,j,k))  ! set sgn, to account for different wind directions
           ...
           if (k < 4 .or. k >= kmax) then
              ...
              if (lsmz) then ! field around this location is smooth -> use regular 5th order (upwind)
                 sgn = sign(real(1.0,kind(w0)),w0(i,j,k))  ! set sgn, to account for different wind directions
