...
contains
  subroutine startup(path)
    ...
    call D_MPI_BCAST_S(nprocx ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(nprocy ,1,0,commwrld,mpierr)

    ! Initialize MPI
    call initmpi
    
  !broadcast namelists
    call D_MPI_BCAST_S(iexpnr     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lwarmstart ,1,0,commwrld,mpierr)
    call D_MPI_BCAST  (startfile  ,50,0,commwrld,mpierr)
    call D_MPI_BCAST  (author     ,80,0,commwrld,mpierr)
    call D_MPI_BCAST_S(runtime    ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(trestart   ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(dtmax      ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(dtav_glob  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ltotruntime,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(wctime     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(timeav_glob,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(nsv        ,1,0,commwrld,mpierr)

    call D_MPI_BCAST_S(itot       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(jtot       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(kmax       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(xsize      ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ysize      ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(xlat       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(xlon       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(xyear      ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(xday       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(xtime      ,1,0,commwrld,mpierr)

    call D_MPI_BCAST_S(z0         ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ustin      ,1,0,commwrld,mpierr)
    !call D_MPI_BCAST_S(lneutraldrag ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(wtsurf     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(wqsurf     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST  (wsvsurf(1:nsv),nsv,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ps         ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(thls       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(chi_half   ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lmoist     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lcoriol    ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lpressgrad ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(igrw_damp  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(geodamptime,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lforce_user,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lmomsubs   ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ltimedep   ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ltimedepuv ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ltimedepsv ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lrigidlid  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(unudge     ,1,0,commwrld,mpierr)

    call D_MPI_BCAST_S(irad       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(timerad    ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(iradiation ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(rad_ls     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(rad_longw  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(rad_shortw ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(rad_smoke  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(useMcIca   ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(rka        ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(dlwtop     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(dlwbot     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(sw0        ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(gc         ,1,0,commwrld,mpierr)
    ! CvH call D_MPI_BCAST_S(sfc_albedo ,1,MY_REAL   ,0,commwrld,mpierr)
    call D_MPI_BCAST_S(reff       ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(isvsmoke   ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lcloudshading,1,0,commwrld,mpierr)

    call D_MPI_BCAST_S(llsadv     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lqlnr      ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(lambda_crit,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(cu         ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(cv         ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ksp        ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(irandom    ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(krand      ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(krandumin  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(krandumax  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(randthl    ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(randqt     ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(randu      ,1,0,commwrld,mpierr)

    call D_MPI_BCAST_S(ladaptive  ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(courant,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(peclet,1,0,commwrld,mpierr)

    call D_MPI_BCAST_S(isurf   ,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(ibas_prf,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(iadv_mom,1,0,commwrld,mpierr)
    ...
    call D_MPI_BCAST_S(solver_id,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(maxiter,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(n_pre,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(n_post,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(tolerance,1,0,commwrld,mpierr)
    call D_MPI_BCAST_S(precond,1,0,commwrld,mpierr)
